document.addEventListener('DOMContentLoaded', function() {
	...
	function updatePregnancyWeekState() {
		...
		if (selected === '출산급여') {
			$pregWeekRadios.prop('checked', false).prop('disabled', true)
					.closest('.form-check').addClass('readonly-box').css(
							'pointer-events', 'none');
		} else {
			$pregWeekRadios.prop('disabled', false).closest('.form-check')
					.removeClass('readonly-box').css('pointer-events', '');
		}
	}
	...
	$btnSave.on('click', function() {
		...
		// 저장 할때 url 경로
		pendingAction = `/approval24/complain/category/mt2/${complainId}`; // 기본 저장 경로
	...
	$btnApprove.on('click', function() {
		...
		// 승인 할때 url 경로
		pendingAction = `/approval24/mt2/${complainId}/approve`;
	...
	$btnReject.on('click', function() {
		...
		// 반려 할때 url 경로
		pendingAction = `/approval24/mt2/${complainId}/reject`;
	...
	$btnCancel.on('click', function() {
		...
		// 취하 할때 url 경로
		pendingAction = `/approval24/mt2/${complainId}/cancel`;
