						...
						<div class="card-body">
							<form id="menuForm"
								...
								<div class="form-group">
									<label for="menuName">메뉴명</label> <input type="text" class="form-control" id="menuName" name="menuName" value="${menu != null ? menu.menuName : ''}" required>
								</div>

								<div class="form-group">
									<label for="menuUrl">URL</label> <input type="text" class="form-control" id="menuUrl" name="menuUrl" value="${menu != null ? menu.menuUrl : ''}">
								</div>
									...
									<div class="d-flex align-items-center">
										<select class="form-control" id="parentMenuId" name="parentMenuId" style="max-width: 300px;">
											...
											<c:forEach var="pm" items="${parentMenus}">
												<option value="${pm.menuId}" <c:if test="${menu != null && pm.menuId == menu.parentMenuId}">selected</c:if>>${pm.menuName}</option>
											</c:forEach>
										</select> <span id="selectedParentId" class="ml-3 text-gray-600 small"> <c:if test="${menu != null && menu.parentMenuId != null}">
                                            선택된 ID: ${menu.parentMenuId}
                                        </c:if> <c:if test="${menu == null || menu.parentMenuId == null}">
                                            선택된 ID: 없음
                                        </c:if>
										</span>
									</div>
								...
								<div class="form-group">
									<label for="seq">정렬순서</label> <input type="number" class="form-control" id="seq" name="seq" value="${menu != null ? menu.seq : '100'}" placeholder="0 이상의 정수만 입력" min="0" required>
								</div>

								<div class="form-group">
									<label for="popupYn">팝업 여부</label> <select class="form-control" id="popupYn" name="popupYn">
	...
	<script>
		document
				.addEventListener(
						'DOMContentLoaded',
						function() {
							const parentSelect = document
									.getElementById('parentMenuId');
