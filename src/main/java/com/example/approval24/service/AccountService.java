...
public class AccountService {
	...
	public List<MenuVO> getAuthMenus(Long accountID) {
		// 권한 ID 조회
		List<Long> authorityIds = authorityAccountDAO.findByAccountId(accountID).stream().map(a -> a.getAuthorityId())
		...
		List<MenuVO> menuVOList = menus.stream().map(menu -> {
			...
			authorityMenus.stream().filter(am -> am.getMenuId() != null && am.getMenuId().equals(menu.getMenuId()))
